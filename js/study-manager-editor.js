...
export function renderCourseList() {
  ...
  const ranking = buildCourseRiskRanking();
...
export function populateCourseForm(id) {
...
export function populateAssessmentForm(id) {
...
export function updateAssessmentStatus(id, status) {
  const assessment = state.assessments.find((item) => item.id === id);
  if (!assessment) return;
